...
for j =1:size(notbinned,2)% loop over colony sizes;
    for k=1:(size(notbinned{j},1)-1)
        for ii = 1:(size(notbinned{j},2))
            if (notbinned{j}(k,ii) > 900) || (notbinned{j}(k,ii) < 100);                        % remove outliers
...
for k=1:ucol
    ...
    ylim([0 400]);title('SD','FontSize',21);xlabel('Frames')
    legend('1','2','3');
...
for k=1:ucol
    ...
    figure(7+k), histogram(nonzeros((lastT{k}(isfinite(lastT{k})))),'normalization','pdf'), legend([ num2str(k) '-cell colony' ]);
...
for k=1:ucol
    ...
    title('SD during last time period','FontSize',21); ylabel('Final signaling SD');
    xlim([0 120]);
    ylim([0 120]);
    hold on
    legend('1','2','3');
