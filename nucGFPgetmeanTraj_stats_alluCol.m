...
for j =1:3%size(notbinned,2)% loop over colony sizes;
    for k=1:(size(notbinned{j},1)-1)
        for ii = 1:(size(notbinned{j},2))
            if (notbinned{j}(k,ii) > 1000) || (notbinned{j}(k,ii) < 100);                        % remove outliers
...
for k=1:3%ucol
    nbins = 10;
    figure(7), subplot(1,ucol,k),histogram(nonzeros((lastT{k}(isfinite(lastT{k})))),nbins,'normalization','pdf','FaceColor',C{k}), legend([ num2str(k) '-cell colony' ]);xlim([100 1000]);
